...
import DialogPrompt from "~features/DialogPrompt"
...
const PlasmoOverlay = () => {
  ...
  //Icons that need to add on linkedIn
  const Icons = () => (
    <button onClick={() => setShowDialogPrompt(true)} style={{ padding: '8px', backgroundColor: "#FFFFFF", borderRadius: "100%" }}>
  ...
  //add Icons to the LinkedIn 
  const addIcons = (messageField: HTMLElement) => {
    const existingContainer = document.getElementById('custom-icons-container');
    if (!existingContainer) {
      const iconsContainer = document.createElement('div');
      iconsContainer.id = 'custom-icons-container';
      iconsContainer.style.display = "flex";
      iconsContainer.style.justifyContent = "end";
      iconsContainer.style.position = "absolute"
      iconsContainer.style.bottom = "10px"
      iconsContainer.style.right = "10px"
      // messageField.appendChild(iconsContainer)
      messageField.parentNode.insertBefore(iconsContainer, messageField.nextSibling);
      const root = createRoot(iconsContainer);
      root.render(<Icons />)
    }
  };

  //setup for event Listerner on messageBox of linkedIn to get the event
  const setupEventListeners = () => {
    const messageField = document.querySelector('.msg-form__contenteditable');
    if (messageField) {
      messageField.addEventListener('focus', () => addIcons(messageField as HTMLElement));
    }
  };

  //setup for the event on the LinkedIn page 
  useEffect(() => {
  ...
  }, [document.body]); //event adding/rendering only at the first load/refresh


  if (ShowDialogPrompt) {
    return (
      <>
        <div className="bg-gray-300 z-10 w-screen h-screen fixed opacity-30" onBlur={() => console.log("clic")}>
        </div>
        <div className="z-50 fixed bottom-1/3 right-1/2 translate-x-1/2 rounded-lg">
          <DialogPrompt setShowDialogPrompt={setShowDialogPrompt} />
        </div>
      </>
    )
  }
